...
const QuizQuestions = ({ name }) => {
  ...
  useEffect(() => {
    ...
    fetch(
      `https://redux-learning-server-production.up.railway.app/userAnswer/${user?.email}`
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  const finalResult = () => {
    if (resultInPercentage > 40) {
      axios
        .post(
          `https://redux-learning-server-production.up.railway.app/userAnswer`,
          userData
        )
        .then((response) => {
          if (response) {
      ...
      // axios
      //   .put(
      //     `https://redux-learning-server-production.up.railway.app/users`,
      //     userData
      //   )
      //   .then((response) => {
      //     if (response) {
