...
class QuillEditor extends React.Component {
    ...
    insertImage = (e) => {
        ...
        if (e.currentTarget && e.currentTarget.files && e.currentTarget.files.length > 0) {
            ...
            axios.post('/doc/uploadFiles', formData, config)
                .then(response => {
                    if (response.data.success) {
                        ...
                        quill.insertEmbed(position, "image", { src: "https://redux-learning-server-production.up.railway.app/" + response.data.url, alt: response.data.fileName });
            ...
            axios.post('/doc/uploadFiles', formData, config)
                .then(response => {
                    if (response.data.success) {
                        ...
                        let range = quill.getSelection();
                        let position = range ? range.index : 0;
                        quill.insertEmbed(position, "video", { src: "https://redux-learning-server-production.up.railway.app/" + response.data.url, title: response.data.fileName });
                        quill.setSelection(position + 1);
